...
nutnetdf_raw <-read.csv(here("output-data", "merged_NutNet_Oct2019_old.csv"), stringsAsFactors = FALSE, colClasses = c("site_code" = "factor")) %>%
  ...
  # Equality is messed up for all taxon when DI when site_code == bogong.au, plot == 3, due to different precision levels
  mutate_if(is.numeric, ~round(.x, digits = 7))
...
# pick out only the columns we're interested in
# this will help me figure out what I'm actually using.
nutnetdf_used <- nutnetdf %>%
  select(site_name, site_code, year_trt, trt, plot, Taxon, DIgroup, DI,
         totplotcover.yr.live, max_cover, live, Family, rich, live_mass,
         rel_abundance_year0)

nutnetdf_full <- nutnetdf_used %>%
  mutate(rel_cover = (max_cover/totplotcover.yr.live )*100) %>%
  as_tibble()

# The plots will focus on live biomass, so this will be our base dataset.
nutnetRel <- nutnetdf_full %>%
  filter(live == 1) 
...
# get mean and max abundance for each Site/Taxon.. ie aggregating over plot and treatment
# ? do we want to add plot to this?
...
# get number of years observed. will be merged in with other stuff below
  # this would be the same as if we used nutnetRel instead of nutnetdf_allspp
...
# Abundance metric = average of mean pretreatment abundance and pretreatment frequency over all plots?
...
# Aggregate over plots. (ie unique row in nutnetPresAbs is everything in group_by + plot)
# presence = The number of plots the taxon was in (within a given year/site/trt)
# PA = 1 if the Taxon was in at least 1 plot (for the given year/site/trt)
...
# ?? This used to have DI in it, but DI is defined by plot, and nutnetPASite is
# aggregated over all plots, so it doesn't really make sense. maybe we want the duplicated groups ??
nutnetPASiteTrt <- nutnetPASite%>%
  filter(trt!='Control')%>%
  left_join(nutnetPACtl, by = c("site_code", "Taxon", "year_trt")) %>%
  ...
  left_join(distinct(di_groups, site_code, Taxon, DIgroup), by = c("site_code", "Taxon"))# %>%
  # note that we're missing abund metric now, so have to add it separate
  #left_join(abund_metric_df, by = c("site_code", "Taxon"))

# Get the number of Taxon that disappear in each site/year/trt
...
nutnetLoss_trtDIyr <- barGraphStats(data=subset(nutnetLossorNotSite, 
...
ggplot(nutnetLoss_trtDIyr, aes(x=year_trt, y=mean, color=as.factor(DIgroup))) +
...
year0_ranef_pred <- cbind(newdata, year0_pred)
...
loss_lag_pred_fix <- predictInterval(multiyear_model_plot_trt,
                                 newdata=newdata,
                                 which="fixed", type="probability",
                                 include.resid.var = FALSE)
