...
# is left join correct? maybe inner join is better.. cover has the taxon, so is at the more coarse level
  # just kidding they're the same on this data.
...
Old way of reading in data (when we were given the merged dataset straight up. now we generate it in the first chunk)
```{r deprecated, eval = FALSE, include=FALSE}
## Reading in data
nutnetdf_old_raw <-read.csv(here("output-data", "merged_NutNet_Oct2019_old.csv"), stringsAsFactors = FALSE, colClasses = c("site_code" = "factor")) %>%
...
nutnetdf_old_used <- nutnetdf_old %>%
...
* Maybe include `plot` in the first group_by? or exclude `site_code`?
  * adding plot won't do anything I think. just adds another level of abstraction (ie we take 3 avrages instead of 2) (It's 2 right now because biomassResp aggregates over all the plots for a treatment, and the summary aggregates over all sites.)
  * The only differences would be in the standard errors, i think? because we're changing the n we're looking at.( right now n is number of sites)
```{r slide6}
...
* plots the whole distibution of the data.
```{r slide7}
...
(abund_trt_di2 <- metric_diff_by_group("rel_cover", "DIgroup", remove_outliers = TRUE) + 
  labs(title = "Abundance by Treatment, over all Sites",
       subtitle = "DI Group 2",
       y = "Abundance Difference (%)",
       x = "Treatment")
