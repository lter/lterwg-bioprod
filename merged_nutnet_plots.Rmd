...
modeling_data_year0 <- nutnetdf_allspp %>% 
  ...
  filter(#year_trt == max(year_trt),
         trt %in% c("Control", "N", "P", "K", "NPK", "NPK+Fence")) %>%
  ungroup()
...
# library(merTools)
# 
# year0_model <- glmer(lost_at_end ~ rel_rank_year0  + (1|site_code/plot), 
#                    data = modeling_data,
#                    family = "binomial")
# 
# ggplot(aes(x = rel_rank_year0, y = lost_at_end, color = trt), data = modeling_data) + 
#   geom_point(position=position_jitter(width=0.05, height=0.05),
#              alpha=0.2, color="grey") +
#     geom_smooth(aes(x = rel_rank_year0, y = year0_pred))
# 
# newdata <- crossing(site_code = modeling_data$site_code %>% unique, 
#                     plot = unique(modeling_data$plot), 
...
* sometimes end up dividing by 0, throwing an error

```{r}
rel_rank_by_year <- nutnetRel %>%
...
treatments <- nutnetRel %>%
  select(trt, site_code, plot, Taxon, year_trt)

# site_code, plot, year, taxon is a unique identifier in our original data (nutnetRel)
modeling_data <- nutnetdf_allspp %>% 
  select(site_code, plot, year_trt, Taxon, rel_cover) %>%
  left_join(rel_rank_by_year, by = c("site_code", "plot", "Taxon", "year_trt")) %>%
  left_join(treatments, by = c("site_code", "plot", "Taxon", "year_trt")) %>%
  # figure out when a species is lost
  group_by(site_code, plot, Taxon, year_trt) %>%
  mutate(lost = ifelse(rel_cover ==0, 1, 0)) %>%
  ungroup() #%>%
  #filter(trt.y %in% c("Control", "N", "P", "K", "NPK", "NPK+Fence")) 

ggplot(aes(x = rel_rank_lag, y = lost, color = trt.y), data = modeling_data) + 
  geom_point(position=position_jitter(width=0.05, height=0.05),
             alpha=0.2, color="grey") +
    geom_smooth(method = "glm", method.args = list(family = "binomial"))
