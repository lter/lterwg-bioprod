...
Currently, we use the `Rich` column given to us by nutnet (In particular, the dataset `comb-by-plot-clim-soil-diversity-09-Apr-2018.csv`). However, in `NutNet_rare_spp.R`, richness is computed, using the `full-biomass-09-June-2017.csv` data. In the code chunk below, we use the merged data (`merged_NutNet_Oct2019.csv`), compute richness using the method outlined in `NutNet_rare_spp.R`, and compare it to the built in `rich` column.
```{r}
...
#### Adapted version, counting unique taxon instead of rows, don't group by plot
richness_taxon <- nutnetRel%>%
  filter(live==1, Family!='NULL')%>%
  group_by(year_trt, site_code,  trt)%>%
  summarise(richness_adapt=length(unique(Taxon)))%>%
  ungroup()


#### Compare with the nutnet built in version.
nutnetRel %>%
  select(year_trt, site_code, plot, trt, rich) %>%
  inner_join(richness_raw, by = c("year_trt", "site_code", "plot", "trt")) %>%
  inner_join(richness_taxon, by = c("year_trt", "site_code", "trt")) %>%
...
nutnetdf_allspp <- nutnetRel %>%
  select(year, site_name, site_code, block, plot, subplot, year_trt, trt, Taxon, DIgroup, rel_cover) %>%
...
(missing_at_end <- ggplot(biotime_loss_summary,
       ...
       aes(x=REL_RANK_T0, y=lost_at_end, group=plot)) +
...
biotime_duo_reshape <- biotime_sum_abund %>%
  dplyr::select(site_code, plot, year, Taxon, rel_cover, DIgroup) %>%
...
(plot_fit_loss_mod <- ggplot(biotime_duo_reshape, 
                             aes(x = lag_rel_abund_rank, y = lost, color = DIgroup)) +
    geom_smooth(method = "glm", method.args = list(family = "binomial")) +  
