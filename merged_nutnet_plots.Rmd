...
# # NATHAN TEST DOING GROUPINGS BY (SITE, TREATMENT) INSTEAD OF AT PLOT LEVEL
# cover_april2020 <- read_csv(here("output-data", "NutNet_FullCoverData_ProcessedApril2020_TrtSiteTest.csv")) %>%
#   select(site_name, site_code, year_trt, trt, plot, Taxon, DIgroup, DIgroup2, RelAbund_group, RelAbund_group2, DI,
#          totplotcover.yr.live, max_cover, live, Family, rel_abundance_year0)
...
nutnetdf <- nutnetdf %>%
  mutate(rel_cover = (max_cover/totplotcover.yr.live )*100,
         rel_abundance_year0 = rel_abundance_year0*100) #%>%
  #as_tibble()
...
nutnetPlot <- nutnetRel %>%
  filter(trt %in% c("Control", "N", "NP", "NPK"))

# we create groupings based on pretreatment statistics -- make a lookup table for these

# nutnetRel_year0 <- nutnetRel %>%
#   filter(year_trt == 0) %>%
#   rename("rel_abundance_year02" = "rel_cover") %>%
#   group_by(site_code, plot) %>%
#   mutate(distinct_di = n_distinct(DI))
#   # mutate(DIgroup = cut(DI, breaks = quantile(DI, probs = c(0., .2, .8, 1.), na.rm = T),
#   #                      labels = c("Rare", "Subordinate", "Dominant")))
# 
# a <- nutnetRel_year0 %>%
#   group_by(site_code, plot) %>%
#   mutate(num_di = n_distinct(DI, na.rm =T),
#          DIgroup4 = ifelse(num_di >=3,
#                           cut(DI,
#                               breaks = jitter(quantile(DI, probs = c(0.0,.2, .8, 1.0), na.rm = T),
#                                               factor = 0.01),
#                               #labels = c("Rare", "Subordinate", "Dominant")
#                               include.lowest = T,
#                               right = F
...
#                           )
#          )
#   # mutate(DIgroup = cut(DI, breaks = unique(quantile(DI, probs = c(0, .2, .8, 1), na.rm = T))))
# 
# two_plots <- a %>%
#   filter(site_code == "azi.cn", plot == 1) %>%
#   select(site_code, plot, DI, num_di, DIgroup4)
...
relAbund1_groups <- nutnetRel %>%
  select(site_code, plot, Taxon, RelAbund_group) %>%
  distinct()

relAbund2_groups <- nutnetRel %>%
  select(site_code, plot, Taxon, RelAbund_group2) %>%
  distinct()
...
nutnetdf_allspp <- nutnetRel %>%
  ...
  left_join(relAbund1_groups, by = c("site_code", "plot", "Taxon")) %>%
  left_join(relAbund2_groups, by = c("site_code", "plot", "Taxon"))


trt_of_interest <- c("N", "NP", "NPK")
```
...
# plot settings
theme_set(theme_bw())
theme_update(axis.title.x=element_text(size=24, vjust=-0.35, margin=margin(t=15)), axis.text.x=element_text(size=24),
             axis.title.y=element_text(size=24, angle=90, vjust=0.5, margin=margin(r=15)), axis.text.y=element_text(size=24),
...
prop_lost_by_yeartrt <- function(grouping){
  ...
  prop_lost_by_yr <- nutnetdf_allspp %>%
    filter(trt %in% c("Control", trt_of_interest),
           year_trt > 0,            # we don't want to say something was lost if it was never there.
           !is.na(!!grouping)) %>%     # DI group was defined at pretreatment levels, so DI group NA means 
                                        # we didn't have pretreatment info.
    group_by(year_trt, trt, !!grouping) %>%
  ...
  point_plot <- ggplot(prop_lost_by_yr, aes(x=year_trt, y=mean_prop_lost,
  ...
  list(prop_df = prop_lost_by_yr,
       prop_plot = point_plot)
}

# by DI group
prop_lost_by_yrTrtDI <- prop_lost_by_yeartrt("DIgroup")
prop_lost_by_yrTrtDI$prop_plot

# by Rel Abund group
prop_lost_by_yrTrtRelAbund <- prop_lost_by_yeartrt("RelAbund_group")
prop_lost_by_yrTrtRelAbund$prop_plot

# Q: Why do the bars get so big at the end?
# A: there are just so much fewer taxon in the later years, so the standard error (sd / sqrt(n)) blows up. also groupings are defined within a (site, plot, year) but these plots display (year, treatment, grouping) and aggregate over plots and sites.
prop_lost_by_yrTrtDI$prop_df %>%
...
prop_lost_by_yrTrtRelAbund$prop_df %>%
  arrange(desc(year_trt)) %>%
  group_by(year_trt, RelAbund_group) %>%
  summarize(num_taxon = sum(n)) %>%
  ggplot() + 
  geom_col(aes(x = year_trt, y = num_taxon)) + 
  facet_wrap(~RelAbund_group) + 
  labs(title = "Number of Taxon Per Treatment Year")
...
(badBiomassFig <- ggplot(biomassResp_summary, 
    ...
    scale_color_discrete(name = "Treatment", labels = c("N", "NP", "NPK"))
  
)

ggsave("nutnet-figD.pdf", badBiomassFig, 
       device = "pdf", 
       path = here("nutnet-figs"),
       width = 10,
       height = 7)


trt_names <- c("N_diff" = "N", "NP_diff" = "NP", "NPK_diff"= "NPK")#, "NPKFence_diff" = "NPK+Fence", "Fence_diff" = "Fence")
...
#' @param last_year is a flag which filters out only the last year.
metric_diff_by_group <- function(metric, grouping, remove_outliers = FALSE, last_year = FALSE){
  metric <- sym(metric)
  grouping <- sym(grouping)
  
  data <- nutnetRel
  
  # if we only want to look at the last year.
  if(last_year){
    data <- data %>%
      group_by(site_code, plot) %>%
  ...
  metric_diff_DItrtSite <- data %>%
    ...
    gather(key=trt, value=diff, c(NPK, NP, N))
...
rich_trt_di1 + rich_trt_di2 + plot_layout(guides = "collect") + 
  plot_annotation(title = "Richness by Treatment, over all Sites")
...
(rich_relAbund1 <- metric_diff_by_group("rich", "RelAbund_group") + 
  labs(title = "Richness by Treatment, over all Sites",
       ...
       y = "Richness Difference (%)",
       x = "Treatment") + 
  expand_limits(y = c(-50, 225))
)

ggsave("nutnet-figA.pdf", rich_relAbund1, 
       device = "pdf", 
       path = here("nutnet-figs"),
       width = 10,
       height = 7)
...
(rich_relAbund1_last_year <- metric_diff_by_group("rich", "RelAbund_group", last_year = T) + 
  labs(title = "Richness by Treatment, over all Sites, Last Year",
       subtitle = "Rel Abund Group 1",
       y = "Richness Difference (%)",
       x = "Treatment") + 
  expand_limits(y = c(-50, 225))
...
ggsave("nutnet-figA_lastYear.pdf", rich_relAbund1_last_year, 
       device = "pdf", 
       path = here("nutnet-figs"),
       width = 10,
       height = 7)
...
abund_trt_di1 + abund_trt_di2 + plot_layout(guides = "collect") + 
  plot_annotation(title = "Abundance by Treatment, over all Sites")
...
ggsave("nutnet-figB.pdf", abund_trt_relAbund1, 
       device = "pdf", 
       path = here("nutnet-figs"),
       width = 10,
       height = 7)

(abund_trt_relAbund1_lastYr <- metric_diff_by_group("rel_cover", "RelAbund_group", remove_outliers = T, last_year = T) + 
  labs(title = "Abundance by Treatment, over all Sites",
       subtitle = "Rel Abund Group 1",
       y = "Abundance Difference (%)",
       x = "Treatment") + 
...
ggsave("nutnet-figB_lastYr.pdf", abund_trt_relAbund1_lastYr, 
       device = "pdf", 
       path = here("nutnet-figs"),
       width = 10,
       height = 7)
...
### DEPRECATED IN FAVOR OF INTERACTION MODEL IN NEXT CHUNK
...
ggsave("nutnet-figC.pdf", missing_at_end_interaction, 
       device = "pdf", 
       path = here("nutnet-figs"),
       width = 10,
       height = 7)
