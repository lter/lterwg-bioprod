...
ggplot(rich_diff_DItrtSite) + 
  geom_boxplot(aes(x = trt, y = diff, fill = DIgroup), na.rm= T) + 
  geom_hline(yintercept = 0) + 
...
* crazy outliers
```{r slide8}
abund_diff_di_wide <- nutnetRel %>%
  filter(year_trt!=0)%>%
  group_by(trt, site_code, DIgroup)%>%
  summarise(live_abund_mean=mean(rel_cover)) %>%
  ungroup()%>%
...
# there are pretty nasty outliers which make the plot hard to read. remove if > 5 MAD
abund_diff_di_long <- abund_diff_di_wide %>%
  select(DIgroup, NPK = NPK_diff, N = N_diff, NP = NP_diff, Fence = Fence_diff, `NPK+Fence` = NPKFence_diff)%>%
  gather(key=trt, value=diff, NPK:`NPK+Fence`) %>%
  na.omit() %>%
  filter(abs(diff) <= 5*mad(diff))

ggplot(abund_diff_di_long) + 
